	...
	public class GlyphGlobalItem : GlobalItem
	{
		...
		public override void PostDrawInInventory(Item item, SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
		{
			...
			if (texture != null)
			{
				...
				spriteBatch.Draw(texture, slotOrigin + offset, null, Color.White, 0f, Vector2.Zero, Main.inventoryScale, SpriteEffects.None, 0f);
			}
		}
		...
		public override void OnHitNPC(Item item, Player player, NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			int baseRarity = Math.Max(item.OriginalRarity, 1);

			if (Glyph == GlyphType.Frost && Main.rand.NextBool((int)MathHelper.Clamp(30 - (item.useTime / 2f), 2, 12)))
				FrostGlyph.FreezeEffect(player, target, null);
			if (Glyph == GlyphType.Void && Main.rand.NextBool((int)MathHelper.Clamp(30 - (item.useTime / 2f), 2, 12)))
				VoidGlyph.VoidCollapse(player, target, null, damageDone);
			...
			{
				if ((mPlayer.genericCounter = MathHelper.Clamp(mPlayer.genericCounter + (item.useTime / 60f), 0, 1)) == 1)
				{
			...
			{
				if ((mPlayer.genericCounter = MathHelper.Clamp(mPlayer.genericCounter + (item.useTime / 60f), 0, 1)) == 1)
					player.AddBuff(ModContent.BuffType<TemporalShift>(), (int)MathHelper.Clamp(item.useTime * 2f, 30, 60));
			}
			...
			if (Glyph == GlyphType.Veil)
				mPlayer.veilCounter = MathHelper.Clamp(mPlayer.veilCounter + (item.useTime / 300f), 0, 1);
		}
		...
		public override void ModifyHitNPC(Item item, Player player, NPC target, ref NPC.HitModifiers modifiers)
		{
			...
			if (Glyph == GlyphType.Rage && mPlayer.frenzyDamage > 0)
			{
				...
				RageGlyph.RageEffect(player, target, null);
			}
		...
		{
			if (Glyph == GlyphType.Storm && player.whoAmI == Main.myPlayer && player.ItemAnimationJustStarted && Main.rand.NextBool((int)MathHelper.Clamp(30 - (item.useTime / 2), 2, 10)))
			{
