...
public class Fathomless_Chest_World : ModSystem
{
	...
	private static void ClearCircle(int i, int j)
	{
	...
	{
		int width = ShrineArray.GetLength(0);
		int height = ShrineArray.GetLength(1);
		...
		i -= 10;
		j -= 16;
		GenVars.structures.AddProtectedStructure(new Rectangle(i, j, width + 5, height + 8));
	}

	private static void PlaceBeams(int i, int j)
	{
	...
	public void FinalShrineGen(int spawnX, int spawnY)
	{
		...
		while (safetyCheck)
		{
			...
			if (!GenVars.structures.CanPlace(new Rectangle(spawnX - radius, spawnY - radius, radius * 2, radius * 2), 12))
				safetyCheck = true;
		}
