...
public static class ParticleHandler
{
	...
	{
		if (Main.netMode == Terraria.ID.NetmodeID.Server || activeParticles == MaxParticlesAllowed || !ModContent.GetInstance<SpiritClientConfig>().ForegroundParticles)
			return;
	...
	internal static void DrawAllParticles(SpriteBatch spriteBatch)
	{
		foreach (Particle particle in particles)
		{
			if (particle == null || particle is ScreenParticle && !ModContent.GetInstance<SpiritClientConfig>().ForegroundParticles)
				continue;
		...
		if (batchedAlphaBlendParticles.Count > 0)
		{
			...
			foreach (Particle particle in batchedAlphaBlendParticles)
			{
				...
				else
				{
					...
					spriteBatch.Draw(particleTextures[particle.Type], particle.Position - Main.screenPosition, null, particle.Color, particle.Rotation, particle.Origin, scale, SpriteEffects.None, 0f);
				}
			}
