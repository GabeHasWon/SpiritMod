...
public class BoidHost : ILoadable
{
	...
	public void Load(Mod mod)
	{
		...
		for (int i = 0; i < flocks; i++)
		{
			...
			Texture2D[] sardineTexture = [ModContent.Request<Texture2D>($"SpiritMod/Textures/AmbientFish/sardine", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value];
			Texture2D[] shrimpTexture = [ModContent.Request<Texture2D>($"SpiritMod/Textures/AmbientFish/shrimp", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value];
