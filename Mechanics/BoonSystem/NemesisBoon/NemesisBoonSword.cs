...
public class NemesisBoonSword : ModProjectile, IDrawAdditive
{
	...
	{
		Texture2D tex = TextureAssets.Projectile[Projectile.type].Value;
		Texture2D tex2 = ModContent.Request<Texture2D>(Texture + "_White").Value;
	...
	private void DrawSword(SpriteBatch spriteBatch, Texture2D tex, float transparency, float scale)
	{
		...
		for (int k = Projectile.oldPos.Length - 1; k >= 0; k--)
		{
			...
			float num107 = (float)Math.Cos(Main.GlobalTimeWrappedHourly % 2.4f / 2.4f * MathHelper.TwoPi) / 2f + 0.5f;
			Color baseColor = new Color(110 - Projectile.alpha, 94 - Projectile.alpha, 25 - Projectile.alpha, 0).MultiplyRGBA(color);

			for (int i = 0; i < 4; i++)
			{
				Color drawColor = baseColor;
				drawColor = Projectile.GetAlpha(drawColor);
				drawColor *= 1.5f - num107;
				drawColor *= (float)Math.Pow((Projectile.oldPos.Length - k) / Projectile.oldPos.Length / 2f, 1.5f);
				Vector2 offset = (i / 4f * MathHelper.TwoPi + Projectile.rotation).ToRotationVector2();
				Vector2 drawPosition = drawPos + offset * (1.5f * num107 + 3f) - Main.screenPosition + new Vector2(0, Projectile.gfxOffY) - Projectile.velocity * i;
				spriteBatch.Draw(tex, drawPosition, null, drawColor * .6f * transparency, oldRotation[k], origin, Projectile.scale * scale, SpriteEffects.None, 0f);
			}
		}
	}
