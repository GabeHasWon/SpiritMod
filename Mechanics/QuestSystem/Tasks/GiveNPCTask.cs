...
public class GiveNPCTask : QuestTask
{
	...
	public GiveNPCTask(int npcType, int giveItem, int stack, LocalizedText text, LocalizedText objectiveKey, bool requireAll = true, bool takeItems = true, int? optionalReward = null) 
		: this(npcType, [giveItem], [stack], text, objectiveKey, requireAll, takeItems, optionalReward)
	{ }
	...
	internal int[] ItemTypes
	{
		get
		{
			int[] ids = new int[_itemIDs.Length];

			for (int i = 0; i < _itemIDs.Length; i++)
				ids[i] = ContentDefinition.GetItemDefinition(_itemIDs[i]);
