	...
	private static void DoConversion(int i, int j)
	{
		...
		if (tile.HasTile)
		{
			...
			if (ModLoader.TryGetMod("SpiritReforged", out Mod mod))
			{
				...
				if (tileType is TileID.HardenedSand or TileID.CorruptHardenedSand or TileID.CrimsonHardenedSand or TileID.HallowHardenedSand)
				{
					tileType = ModContent.TileType<SpiritDirt>();
				}
				...
				{
					if (tileType == elephantGrass.Type || tileType == corruptElephantGrass.Type || tileType == crimsonElephantGrass.Type || tileType == hallowElephantGrass.Type)
						tileType = ModContent.TileType<SpiritTallgrass>();
				}
