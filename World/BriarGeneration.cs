...
public class BriarGeneration : ModSystem
{
	...
	private static Point16 AdventurerPosition = Point16.Zero;
	...
	private readonly struct BlockSpot(int x, int y, float s)
	{
		public readonly int X = x;
		public readonly int Y = y;
		public readonly float Strength = s;
	}

	private struct LinePoint(float x, float y, float r, float j)
	{
		public readonly Vector2 Point = new(x, y);
		public readonly float Jaggedness = j;

		public float Radius = r;

		public override readonly string ToString() => Point.X + ", " + Point.Y + " -- " + Radius + " -- " + Jaggedness;
	}

	private class LineSection
	{
		public static readonly List<ushort> DO_NOT_BREAK =
		[
			...
			for (int x = minX; x <= maxX; x++)
			{
				for (int y = minY; y <= maxY; y++)
				{
					if (ShouldClear(x, y, noise, maxDist))
					{
						...
						if (tile.HasTile && DO_NOT_BREAK.Contains(tile.TileType)) 
							continue;
	...
	private static Vector2 PointOnEllipse(float angle, Vector2 ellipseCenter, Vector2 ellipseSize)
	{
	...
	private static float GetArcEllipse(float angle, List<float> estimates)
	{
	...
	private static int FindSurfaceAt(int x, int checkAmt = 15, bool ignoreTrees = true)
	{
	...
	private static bool TryPlaceAdventurer(int x, int y)
	{
		...
		AdventurerPosition = new Point16(x, y);
		return true;
	}

	private static void AdventurerTorches(int x, int y)
	{
		int repeats = 0;
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			int x2 = x + WorldGen.genRand.Next(-10, 10);
			int y2 = y + WorldGen.genRand.Next(-10, 10);
			Tile tile = Main.tile[x2, y2];

			if (!tile.HasTile)
				WorldGen.PlaceTile(x2, y2, TileID.Torches, true, false, -1, 3);
	...
	public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
	{
		...
		tasks.Insert(tasks.Count - 3, new PassLegacy("Adventurer Torches", (_, _) =>
		{
			if (AdventurerPosition != Point16.Zero)
				AdventurerTorches(AdventurerPosition.X, AdventurerPosition.Y);
		}));
	}
