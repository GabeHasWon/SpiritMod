...
public class TrueCrimbine : ModItem
{
	...
	public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
	{
		...
		if (charger >= 3)
		{
			Projectile.NewProjectile(source, position.X - 8, position.Y + 8, velocity.X + Main.rand.Next(-230, 230) * 0.004f, velocity.Y + (Main.rand.Next(-230, 230) * 0.004f), ModContent.ProjectileType<GiantBlood>(), damage, knockback, player.whoAmI, 0f, 0f);
			charger = 0;
		}
