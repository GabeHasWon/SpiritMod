...
namespace SpiritMod.Items.Sets.CryoliteSet.CryoSword
{
	public class CryoSwordProj : ModProjectile
	{
		...
		public override void AI()
		{
			...
			int degrees = (int)((double)(UseCounter / player.itemAnimationMax * -swingRange) + 55) * player.direction * (int)player.gravDir;
			if (ReverseSwing)
				degrees = (int)((float)Math.PI - (int)((double)(UseCounter / player.itemAnimationMax * -swingRange) + 55) * player.direction * (int)player.gravDir);
			
			degrees -= startRange * player.direction;
			
			if (player.direction == 1)
				degrees += swingRange;

			Radians = degrees * (Math.PI / swingRange);
			UseCounter = MathHelper.Lerp((float)UseCounter, player.itemAnimationMax, 0.1f);
			...
			for (int i = 0; i < 2; i++)
			{
				...
				if (i > 0)
				{
					...
					for (int k = 0; k < Projectile.oldPos.Length; k++)
					{
						...
						Color trailCol = Projectile.GetAlpha(color.MultiplyRGBA(Color.LightBlue)) * ((float)(Projectile.oldPos.Length - k) / (float)Projectile.oldPos.Length) * (quoteant + 0.25f);
