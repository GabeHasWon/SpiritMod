...
namespace SpiritMod.Items.Weapon.Magic.RealityQuill
{
	public class RealityQuillProjectileTwo : ModProjectile
	{
			...
			foreach (Vector2 point in points)
			{
				if (prev != Vector2.Zero)
				{
					...
					if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), point, prev, 20, ref collisionPoint))
					{
						...
						for (int i = 0; i < 3; i++)
						{
							Vector2 vel = Vector2.Normalize(point - prev).RotatedBy(Main.rand.NextFloat(-0.3f, 0.3f));
							vel = vel.RotatedBy(Main.rand.NextFloat(-0.5f, 0.5f)) * Main.rand.NextFloat(2, 5);
		...
		public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
		{
			...
			modifiers.FinalDamage *= MathHelper.Clamp((float)Math.Sqrt(distance), 1, 3);
		}
