	...
	public class SpiritStar : ModProjectile
	{
		...
		public override void AI()
		{
			for (int i = 0; i < 8; i++)
			{
				...
				Main.dust[index2].position = Projectile.Center - Projectile.velocity / 5 * i;
		...
		{
			const int ShatterProjectileCount = 2;
			
			int deviation = Main.rand.Next(0, 180);
			for (int i = 0; i < ShatterProjectileCount; i++)
			{
				float rotation = MathHelper.ToRadians(270 / ShatterProjectileCount * i + deviation);
