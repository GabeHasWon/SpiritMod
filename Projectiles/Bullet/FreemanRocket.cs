...
public class FreemanRocket : BaseRocketProj
{
	...
	public override void AI()
	{
		...
		{
			if (proj.Hitbox.Intersects(Projectile.Hitbox) && Projectile.whoAmI != proj.whoAmI && proj.type == ModContent.ProjectileType<FreemanRocket>())
			{
		...
		if (Main.myPlayer == Projectile.owner && Projectile.ai[0] <= 0f)
		{
			...
			{
				float num2353 = 8f;
				Vector2 vector329 = Projectile.Center;
			...
			else if (Projectile.ai[0] <= 0f)
			{
				...
				{
					float num2337 = 12f;
					Vector2 vector328 = Projectile.Center;
					float num2336 = Main.mouseX + Main.screenPosition.X - vector328.X;
					float num2335 = Main.mouseY + Main.screenPosition.Y - vector328.Y;
					...
					{
						vector328 = Main.player[Projectile.owner].Center;
						num2336 = Projectile.position.X + Projectile.width * 0.5f - vector328.X;
						num2335 = Projectile.position.Y + Projectile.height * 0.5f - vector328.Y;
