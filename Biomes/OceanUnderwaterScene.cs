...
namespace SpiritMod.Biomes
{
	internal class OceanUnderwaterScene : ModSceneEffect
	{
		...
		public override bool IsSceneEffectActive(Player player) => player.ZoneBeach && !MyWorld.luminousOcean && (player.GetSpiritPlayer().Submerged(30, out int _, true) || player.ZoneDirtLayerHeight || player.ZoneRockLayerHeight) && DetectThoriumCompat(player);
