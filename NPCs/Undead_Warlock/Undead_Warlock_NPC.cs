	...
	public class Undead_Warlock_NPC : GlobalNPC
	{
		...
		public override void PostDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			if (isNecrofied)
			{
				...
				Color color2 = new Color((int) sbyte.MaxValue - npc.alpha, (int) sbyte.MaxValue - npc.alpha, (int) sbyte.MaxValue - npc.alpha, 0).MultiplyRGBA(Microsoft.Xna.Framework.Color.PaleGreen) * 0.7f;
				for (int index2 = 0; index2 < num7; ++index2)
				{
					Color newColor2 = color2;
					Color color3 = npc.GetAlpha(newColor2) * (1f - num8) * 0.7f;
					...
					Main.spriteBatch.Draw(TextureAssets.Npc[npc.type].Value, position2, new Rectangle?(npc.frame), color3, npc.rotation, vector2_3, npc.scale, spriteEffects, 0.0f);
				}
				Main.spriteBatch.Draw(TextureAssets.Npc[npc.type].Value, position1, new Rectangle?(npc.frame), color2, npc.rotation, vector2_3, npc.scale, spriteEffects, 0.0f);
			}
			if (isMagicalShrapneled)
			{
				int num7 = 16;
				...
				Vector2 vector2_3 = new Vector2((float) (TextureAssets.Npc[npc.type].Value.Width / 2), (float) (TextureAssets.Npc[npc.type].Value.Height / Main.npcFrameCount[npc.type] / 2));
				Vector2 position1 = npc.Center - Main.screenPosition - new Vector2((float) texture.Width, (float) (texture.Height / Main.npcFrameCount[npc.type])) * npc.scale / 2f + vector2_3 * npc.scale + new Vector2(0.0f, addY + addHeight + npc.gfxOffY);
				Color color2 = new Color((int) sbyte.MaxValue - npc.alpha, (int) sbyte.MaxValue - npc.alpha, (int) sbyte.MaxValue - npc.alpha, 0).MultiplyRGBA(Color.MediumPurple) * 0.7f;
				
				for (int index2 = 0; index2 < num7; ++index2)
				{
					Vector2 position2 = new Vector2 (npc.Center.X,npc.Center.Y-2) + ((float) ((double) index2 / (double) num7 * 6.28318548202515) + npc.rotation + num10).ToRotationVector2() * 2f - Main.screenPosition - new Vector2((float) texture.Width, (float) (texture.Height / Main.npcFrameCount[npc.type])) * npc.scale / 2f + vector2_3 * npc.scale + new Vector2(0.0f, addY + addHeight + npc.gfxOffY);
					Main.spriteBatch.Draw(TextureAssets.Npc[npc.type].Value, position2, new Rectangle?(npc.frame), color2, npc.rotation, vector2_3, npc.scale, spriteEffects, 0.0f);
				}
				Main.spriteBatch.Draw(TextureAssets.Npc[npc.type].Value, position1, new Rectangle?(npc.frame), color2, npc.rotation, vector2_3, npc.scale, spriteEffects, 0.0f);
			}
		}
