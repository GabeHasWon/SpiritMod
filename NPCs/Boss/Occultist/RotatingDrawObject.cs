...
{
	private List<RotatingDrawObject> _rotatingDrawObjects = [];

	public void AddObject(Texture2D Texture, float YPos, float Radius, float Scale = 1f, Color? Color = null, int MaxTime = -1, Rectangle? DrawFrame = null, float Period = 30, float StartingOffset = 0, 
		float Rotation = 0) => _rotatingDrawObjects.Add(new RotatingDrawObject(Texture, YPos, Radius, Scale, Color, MaxTime, DrawFrame, Period, StartingOffset, Rotation));
