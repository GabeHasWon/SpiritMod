			...
			if (AiTimer > attacktelegraphtime + chargetime)
			{
				...
				if (AiTimer % 6 == 0)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), spawnPos, velocity, ModContent.ProjectileType<OccultistHand>(), NPCUtils.ToActualDamage(40, 1.5f, 2f), 1f, Main.myPlayer, amplitude, periodOffset);
						proj.netUpdate = true;
					}
				}
			}
		...
		private void DaggersP1(Player target)
		{
			...
			if (AiTimer > daggerspintime && AiTimer % 5 == 0 && AiTimer <= daggerspintime + 30)
			{
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), spawnPos, Vector2.Zero, ModContent.ProjectileType<OccultistDagger>(), NPCUtils.ToActualDamage(40, 1.5f, 2f), 1f, Main.myPlayer, angle);
					proj.netUpdate = true;
				}
			}
		...
		private void SoulsP1(Player Target)
		{
			...
			if (AiTimer <= 30)
			{
				...
				if (AiTimer % 15 == 0)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
						Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center + vel, vel.RotatedByRandom(MathHelper.Pi / 12), ModContent.ProjectileType<OccultistSoul>(), NPCUtils.ToActualDamage(30, 1.5f, 2f), 1, Main.myPlayer, Main.rand.NextBool() ? -1 : 1, Target.whoAmI).netUpdate = true;
		...
		private void BruteP1(Player Target)
		{
			...
			if (AiTimer <= chargeTime + bruteSlamTime)
			{
				...
				if (AiTimer == chargeTime)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Bottom, Vector2.Zero, ModContent.ProjectileType<BruteSlam>(), NPCUtils.ToActualDamage(60, 1.5f, 2f), 1, Main.myPlayer, NPC.whoAmI);
					...
					UpdateYFrame(8, 0, 8, delegate (int frameY)
					{
						if (frameY == 4 && Main.netMode != NetmodeID.MultiplayerClient && timer < 150)
						{
							...
							for (int i = -1; i <= 1; i++)
							{
								...
								Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), spawnPos, NPC.direction * Vector2.UnitX * 2.25f, ModContent.ProjectileType<OccultistHandFiery>(), NPCUtils.ToActualDamage(40, 1.5f, 2f), 1f, Main.myPlayer, amplitude * i, 60).netUpdate = true;
							}
			...
			else
			{
				...
				if (timer % 7 == 0 && Main.netMode != NetmodeID.Server && timer > daggerstarttime)
				{
					var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center + Main.rand.NextVector2CircularEdge(30, 50), Vector2.Zero,
						ModContent.ProjectileType<OccultistDagger>(), NPCUtils.ToActualDamage(40, 1.5f, 2f), 1f, Main.myPlayer,
			...
			else
			{
				...
				if (timer % 12 == 0 && timer <= 60)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center + vel * Main.rand.NextFloat(10, 20), vel, ModContent.ProjectileType<OccultistSoul>(), NPCUtils.ToActualDamage(30, 1.5f, 2f), 1, Main.myPlayer, NPC.whoAmI, target.whoAmI).netUpdate = true;
					}
				}
