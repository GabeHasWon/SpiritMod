...
namespace SpiritMod.NPCs.MoonjellyEvent
{
	public class ExplodingMoonjelly : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (NPC.Distance(target.Center) <= 450 || NPC.life < NPC.lifeMax)
				aggro = true;
			else
				aggro = false;
		...
		public override void OnHitPlayer(Player target, Player.HurtInfo hurtInfo)
		{
			...
			int p = Projectile.NewProjectile(NPC.GetSource_FromAI(), target.Center.X, target.Center.Y, 0f, 0f, ModContent.ProjectileType<UnstableWisp_Explosion>(), 15, 0f, Main.myPlayer);
