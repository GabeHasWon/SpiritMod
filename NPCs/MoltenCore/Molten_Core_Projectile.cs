...
namespace SpiritMod.NPCs.MoltenCore
{
	public class Molten_Core_Projectile : ModProjectile
	{
		...
		private readonly int killtimerMax = 270;
		...
		public override void OnHitPlayer (Player target, Player.HurtInfo info) => target.AddBuff(24, 60 * 3);
		...
		public override void PostDraw(Color lightColor)
		{
			if (!Projectile.wet)
			{
				...
				{
					Color color2 = Color.Lerp(Color.Gold, Color.OrangeRed, amount);
					...
					Main.EntitySpriteDraw(texture2D, new Vector2((float) ((double) Projectile.oldPos[index].X - (double) Main.screenPosition.X + (double) (Projectile.width / 2) - (double) TextureAssets.Projectile[Projectile.type].Value.Width * (double) Projectile.scale / 2.0 + (double) vector2_3.X * (double) Projectile.scale) + addWidth, (float) ((double) Projectile.oldPos[index].Y - (double) Main.screenPosition.Y + (double) Projectile.height - (double) TextureAssets.Projectile[Projectile.type].Value.Height * (double) Projectile.scale / (double) 1 + 4.0 + (double) vector2_3.Y * (double) Projectile.scale) + addHeight), new Microsoft.Xna.Framework.Rectangle?(rectangle), color2 * Projectile.Opacity, num2 - 1.57f, origin, MathHelper.Lerp(0.1f, 1.2f, (float) ((10.0 - (double) index) / 40.0)), spriteEffects);
				}
			}
		}
		...
		public override void AI()
		{
			...
			int fadeTime = 20;
			if (killtimer > (killtimerMax - fadeTime))
