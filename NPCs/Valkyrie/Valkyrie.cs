	...
	public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
	{
		...
		if (trailing)
		{
			for (int k = 0; k < NPC.oldPos.Length; k++)
			{
				...
				Color trailColor = color * ((NPC.oldPos.Length - k) / (float)NPC.oldPos.Length / 2);

				spriteBatch.Draw(TextureAssets.Npc[Type].Value, drawPos, NPC.frame, trailColor, NPC.rotation, origin, NPC.scale, effects, 0f);
			}
		}

		spriteBatch.Draw(TextureAssets.Npc[Type].Value, NPC.Center - screenPos + new Vector2(0, NPC.gfxOffY), NPC.frame, NPC.GetAlpha(color), NPC.rotation, origin, NPC.scale, effects, 0);
		return false;
	}
